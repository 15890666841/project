...
//修改
   ...
   // $data = DB::table('sale')         //将两张表拼接起来  
   //              ->join('cate', function($join)  
   //              {  
   //                  $join->on('sale.id', '=', 'cate.id');  
   //              })->select('sale.id', 'cate.id')  
   //                  // ->skip($pageSize*($pageIndex))   //$pageIndex是页码  <span style="color:#ff0000;"><strong><u>即一次会跳过几个数据</u></strong></span>  
